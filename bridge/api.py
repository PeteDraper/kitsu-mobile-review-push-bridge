...
class UnregisterTokenRequest(BaseModel):
    device_token: str
    kitsu_token: str | None = None  # optional — device_token is sufficient to identify the registration

    @field_validator("device_token")
    @classmethod
    def normalise_device_token(cls, v: str) -> str:
        # Lowercase to match the normalisation applied on registration.
        # iOS may return the APNs token in uppercase; the DB always stores
        # lowercase (enforced by RegisterTokenRequest), so the DELETE must
        # use the same casing or it will silently match 0 rows.
        return v.lower()
